pageinfo = [[0,0,0,0,null],
	...
	[0,0,0,0,null],
	[0,0,0,0,null]];
pagedata = [ ["./10_1-purpose-of-this-chapter.html","Introduction &#62; Architecture Overview &#62; Chapter 10 – Devices and Memory-Mapped I/O (MMIO) &#62; 10.1 Purpose of This Chapter","This chapter defines how ASA-EmulatR models devices and memory-mapped I/O (MMIO), and how device interactions integrate with the AlphaCPU pipeline...",""],
...
["./10_13-summary.html","Introduction &#62; Architecture Overview &#62; Chapter 10 – Devices and Memory-Mapped I/O (MMIO) &#62; 10.13 Summary","The ASA-EmulatR device and MMIO subsystem provides a strongly ordered, interrupt-driven device model that matches Alpha AXP hardware behavior.",""],
["./10_2-design-philosophy.html","Introduction &#62; Architecture Overview &#62; Chapter 10 – Devices and Memory-Mapped I/O (MMIO) &#62; 10.2 Design Philosophy","ASA-EmulatR follows real Alpha hardware principles for device modeling:",""],
["./10_3-device-model-overview.html","Introduction &#62; Architecture Overview &#62; Chapter 10 – Devices and Memory-Mapped I/O (MMIO) &#62; 10.3 Device Model Overview","10.3.1 What a Device Is  In ASA-EmulatR, a device is an object that owns one or more MMIO address ranges, exposes registers via MMIO, execute...",""],
...
["./11_1-purpose-of-this-chapter.html","Introduction &#62; Architecture Overview &#62; Chapter 11 - Architectural Invariants &#62; 11.1 Purpose of This Chapter","This chapter defines the architectural invariants of ASA-EmulatR.",""],
...
["./11_11-summary.html","Introduction &#62; Architecture Overview &#62; Chapter 11 - Architectural Invariants &#62; 11.11 Summary","Architectural invariants define what ASA-EmulatR is. They are the foundation on which all future features must stand.",""],
...
["./12_1-purpose-of-this-chapter.html","Introduction &#62; Architecture Overview &#62; Chapter 12 – AlphaCPU Core &#62; 12.1 Purpose of This Chapter","This chapter defines the AlphaCPU core object and its responsibilities within ASA-EmulatR. This is the first chapter of Group 2 (Core Processor Im...",""],
...
["./12_13-summary.html","Introduction &#62; Architecture Overview &#62; Chapter 12 – AlphaCPU Core &#62; 12.13 Summary","AlphaCPU is the sole execution authority in ASA-EmulatR. Nothing executes unless AlphaCPU allows it.",""],
...
["./15_1-memory-is-shared_-not-own.html","Introduction &#62; Architecture Overview &#62; Chapter 15 – Memory System Implementation Details &#62; 15.1 Memory Is Shared, Not Owned","ASA EmulatR implements a single shared physical address space, called GuestMemory, accessible by all CPUs. There is one unified physical address s...",""],
...
["./15_5-load-and-store-semantics.html","Introduction &#62; Architecture Overview &#62; Chapter 15 – Memory System Implementation Details &#62; 15.5 Load and Store Semantics","15.5.1 Loads – Synchronous and Visible  All loads in ASA EmulatR complete synchronously during the EX stage. The MBox calls GuestMemory::read...",""],
...
["./16_1-device-differentiation-an.html","Introduction &#62; Architecture Overview &#62; Chapter 16 – Device Model &#38; DMA &#62; 16.1 Device Differentiation and Class Hierarchy","16.1.1 Device Classes  ASA-EmulatR distinguishes devices by function, bus attachment, and I/O characteristics. The mmio_DeviceClass enumerati...",""],
...
["./16_4-asynchronous-device-threa.html","Introduction &#62; Architecture Overview &#62; Chapter 16 – Device Model &#38; DMA &#62; 16.4 Asynchronous Device Threading (QThread Model)","16.4.1 Threading Architecture  ASA-EmulatR uses Qt&#39;s QThread framework for all concurrent execution. Each AlphaCPU runs in its own QThread (m...",""],
["./16_5-scsi-subsystem.html","Introduction &#62; Architecture Overview &#62; Chapter 16 – Device Model &#38; DMA &#62; 16.5 SCSI Subsystem","16.5.1 SCSI Architecture in ASA-EmulatR  The SCSI subsystem follows a layered structure that separates protocol types from bus controllers an...",""],
["./16_6-tape-drive-emulation-and-.html","Introduction &#62; Architecture Overview &#62; Chapter 16 – Device Model &#38; DMA &#62; 16.6 Tape Drive Emulation and Format Variants","16.6.1 Virtual Tape Architecture  ASA-EmulatR emulates SCSI tape drives via the SCSITape class, which presents standard sequential-access SCS...",""],
...
["./16_8-irq-integration-architect.html","Introduction &#62; Architecture Overview &#62; Chapter 16 – Device Model &#38; DMA &#62; 16.8 IRQ Integration Architecture","16.8.1 IRQ System Migration  The device interrupt architecture in ASA-EmulatR has been migrated away from the legacy IRQController class. Dev...",""],
...
["./17_4-spam-tlb-cache-architectu.html","Introduction &#62; Architecture Overview &#62; Chapter 17 – Address Translation, TLB, and PTE &#62; 17.4 SPAM TLB Cache Architecture","17.4.1 What SPAM Is  SPAM (Set Prediction and Access Memory) is the name for the per-CPU PTE cache in ASA-EmulatR. It models the Alpha EV6 Tr...",""],
...
["./18_1-exception-classification.html","Introduction &#62; Architecture Overview &#62; Chapter 18 – Fault Dispatcher &#38; Precise Exceptions &#62; 18.1 Exception Classification","18.1.1 Two-Level Classification  Every exceptional event in ASA-EmulatR carries a two-level classification. The high-level PendingEventKind (...",""],
...
["./18_6-pipeline-fault-detection-.html","Introduction &#62; Architecture Overview &#62; Chapter 18 – Fault Dispatcher &#38; Precise Exceptions &#62; 18.6 Pipeline Fault Detection and Delivery Flow","18.6.1 Detect-Early / Deliver-Late Model  ASA-EmulatR implements the Alpha precise exception model through a detect-early/deliver-late patter...",""],
...
["./19_5-ipi-architecture.html","Introduction &#62; Architecture Overview &#62; Chapter 19 – Interrupt Architecture &#38; IPI &#62; 19.5 IPI Architecture","19.5.1 Design Decision: Unified Interrupt Path  ASA-EmulatR intentionally does not create a second interrupt system for IPIs. Instead, IPIs a...",""],
...
["./21_1-determinism-as-a-design-g.html","Introduction &#62; Architecture Overview &#62; Chapter 21 – Debugging, Tracing, and Determinism &#62; 21.1 Determinism as a Design Goal","21.1.1 Definition  Determinism in ASA-EmulatR means: given the same initial state, with the same inputs, and the same configuration, the emul...",""],
...
["./21_2-sources-of-nondeterminism.html","Introduction &#62; Architecture Overview &#62; Chapter 21 – Debugging, Tracing, and Determinism &#62; 21.2 Sources of Nondeterminism","ASA-EmulatR explicitly identifies potential nondeterminism sources and applies control strategies to each:",""],
...
["./21_8-deterministic-replay.html","Introduction &#62; Architecture Overview &#62; Chapter 21 – Debugging, Tracing, and Determinism &#62; 21.8 Deterministic Replay","ASA-EmulatR supports deterministic replay by recording external inputs (console keystrokes, device events), interrupt injection points (cycle numb...",""],
...
["./22_1-definition-of-correctness.html","Introduction &#62; Architecture Overview &#62; Chapter 22 – Testing, Validation, and Architectural Compliance &#62; 22.1 Definition of Correctness","Correctness in ASA-EmulatR is defined architecturally, not microarchitecturally. The emulator claims correctness when:",""],
["./22_10-known-deviations-and-non.html","Introduction &#62; Architecture Overview &#62; Chapter 22 – Testing, Validation, and Architectural Compliance &#62; 22.10 Known Deviations and Non-Goals","ASA-EmulatR explicitly documents its deviations from silicon behavior and its non-goals. These are documented choices, not omissions.",""],
...
["./22_12-final-compliance-stateme.html","Introduction &#62; Architecture Overview &#62; Chapter 22 – Testing, Validation, and Architectural Compliance &#62; 22.12 Final Compliance Statement","ASA-EmulatR is not validated by optimism or anecdote. It is validated by deterministic execution, layered testing, explicit invariants, and archit...",""],
...
["./22_9-architectural-compliance-.html","Introduction &#62; Architecture Overview &#62; Chapter 22 – Testing, Validation, and Architectural Compliance &#62; 22.9 Architectural Compliance Matrix","ASA-EmulatR maintains a living compliance matrix mapping Alpha AXP architectural requirements to implementation status. Each requirement from the ...",""],
...
["./4_1-purpose-of-this-chapter.html","Introduction &#62; Architecture Overview &#62; Chapter 4 - Functional Execution Domains (\"Boxes) &#62; 4.1 Purpose of This Chapter","This chapter defines the Functional Execution Domains, commonly referred to as \"Boxes\", used throughout the ASA-EmulatR Alpha AXP emulat...",""],
...
["./5_2-purpose-of-this-chapter.html","Introduction &#62; Architecture Overview &#62; Chapter 5 - Memory System Architecture &#62; 5.1 Purpose of This Chapter","This chapter defines the memory architecture of ASA-EmulatR. It explains how memory is modeled, accessed, translated, ordered, and shared across C...",""],
["./6_1-purpose-of-this-chapter.html","Introduction &#62; Architecture Overview &#62; Chapter 6 - Serialization and Stall Model &#62; 6.1 Purpose of This Chapter","This chapter defines how the ASA-EmulatR emulator enforces ordering, visibility, and completion in an otherwise weakly ordered execution model.",""],
["./7_1-purpose-of-this-chapter.html","Introduction &#62; Architecture Overview &#62; Chapter 7 - Exceptions, Faults, and Interrupts &#62; 7.1 Purpose of This Chapter","This chapter defines how the ASA-EmulatR emulator detects, prioritizes, delivers, and resolves exceptions, faults, and interrupts while maintainin...",""],
...
["./8_1-purpose-of-this-chapter.html","Introduction &#62; Architecture Overview &#62; Chapter 8 - PAL and Privileged Boundary &#62; 8.1 Purpose of This Chapter","This chapter defines the Privileged Architecture Library (PAL) boundary and explains how the ASA-EmulatR emulator enforces architectural privilege...",""],
...
["./8_13-summary.html","Introduction &#62; Architecture Overview &#62; Chapter 8 - PAL and Privileged Boundary &#62; 8.13 Summary","The ASA-EmulatR PAL implementation provides a precise, verifiable privileged boundary that mirrors Alpha AXP hardware behavior.",""],
["./9_1-purpose-of-this-chapter.html","Introduction &#62; Architecture Overview &#62; Chapter 9 - SMP Architecture &#62; 9.1 Purpose of This Chapter","This chapter defines how ASA-EmulatR implements symmetric multiprocessing (SMP) while preserving Alpha AXP architectural guarantees.",""],
...
["./9_14-summary.html","Introduction &#62; Architecture Overview &#62; Chapter 9 - SMP Architecture &#62; 9.13 Summary","The ASA-EmulatR SMP architecture provides correct, deterministic multiprocessor behavior that matches Alpha AXP specifications.",""],
["./acronyms--usage-appendix.html","Introduction &#62; Appendix &#62; Acronyms &#38; Usage","===============================================================================",""],
["./alpha-boot-sequence.html","Introduction &#62; Appendix &#62; Keep &#62; Alpha Boot Sequence","╔══════════════════════════════════════════════════════════════════╗",""],
["./alpha-physical-address-space.html","Introduction &#62; Memory &#38; Synchronization Internals &#62; Alpha Physical Address Space","================================================================================",""],
["./alpha-pipeline-.html","Introduction &#62; Appendix &#62; Alpha &#62; BOX Architecture &#62; Alpha PipeLine ","The pipeline is responsible for executing the Instruction Grain life-cycle.  All processing adjacent to it is a side-effect to support the pr...",""],
...
["./alpha.html","Introduction &#62; Appendix &#62; Alpha","Namespace: Alpha  Element Description Header SizeClassId (enum)  alpha_pte_core.h PageSizeCode (enum) // Portable page size encoding alp...",""],
["./alphacpu-alpha.html","Introduction &#62; Appendix &#62; Alpha &#62; AlphaCPU","Primary Responsibility. CPU is sole authority on architectural PC (Architectural Commit)  Rules.  Published Interfaces. Alpha PipeLine &#38;...",""],
["./alphacpu-core.html","Introduction &#62; Architecture Overview &#62; Chapter 12 – AlphaCPU Core","Excellent — here is **Chapter 12**, written as the **first chapter of Group 2 (Core Processor Implementation)**. This chapter is intentionally *me...",""],
["./alphacpudependency.html","Introduction &#62; Appendix &#62; Dependency Requirements &#62; AlphaCPU","Instruction Semantics CPU-REQ-001: For every LL instruction: Perform load from SafeMemory; Invoke ReservationManager::reserve(cpu, pa). CPU-REQ-00...",""],
["./alphamemorysystem-managers.html","Introduction &#62; Appendix &#62; Alpha &#62; Managers &#62; AlphaMemorySystem","Page Table Support: The Page Table Support section in the updated AlphaMemorySystem.h file looks good. It includes the necessary functions for han...",""],
["./alphapipeline-implementation.html","Introduction &#62; Architecture Overview &#62; Chapter 13 – AlphaPipeline Implementation","Below is a **clean, final, drop-in Chapter 13 – AlphaPipeline Implementation**, written in the same tone, rigor, and discipline as Chapter 12.",""],
["./alphasystemmemoryfinakl.html","Introduction &#62; Appendix &#62; Alpha &#62; Managers &#62; AlphaSystemMemoryFinakl","Agreed — this is exactly the right moment to **fix it and formally lock the contract down**.",""],
["./alpha_alu_inl.html","Introduction &#62; Appendix &#62; Function Index &#62; CoreLib &#62; alpha_alu_inl","Namespace: alpha_alu  Returned Function  Description / Usage s64 sext32 Sign-extend a 32-bit value to 64-bit signed integer (Alpha 32-bi...",""],
["./alpha_fp_helpers_inl.html","Introduction &#62; Appendix &#62; Function Index &#62; CoreLib &#62; alpha_fp_helpers_inl","Legacy:  Returned Function Name Inferred description void applyVariantRoundingMode Set IEEE floating-point rounding mode based on Alpha FP va...",""],
["./alpha_int_byteops_inl.html","Introduction &#62; Appendix &#62; Function Index &#62; CoreLib &#62; alpha_int_byteops_inl","This header is a canonical Alpha byte-ops implementation:",""],
["./alpha_int_helpers_inl.html","Introduction &#62; Appendix &#62; Function Index &#62; CoreLib &#62; alpha_int_helpers_inl","This header cleanly separates pure arithmetic semantics from:",""],
["./alpha_operate_opcode_helper.html","Introduction &#62; Appendix &#62; Function Index &#62; CoreLib &#62; alpha_operate_opcode_helper","This helper is decode-time logic, not execution logic.",""],
["./alpha_spam_manager.html","Introduction &#62; Appendix &#62; Alpha &#62; Managers &#62; Alpha_SPAM_Manager","PTEs are cached in the TLB entries via pteRaw in SPAMEntry.",""],
["./alpha_sse_fp_inl.html","Introduction &#62; Appendix &#62; Function Index &#62; CoreLib &#62; alpha_SSE_fp_inl","Returned Function Name Inferred description quint64 add64 Add unsigned 64-bit integers using SSE2 when available and update FPCR on overflow qint6...",""],
["./alpha_sse_int_inl.html","Introduction &#62; Appendix &#62; Function Index &#62; CoreLib &#62; alpha_SSE_int_inl","Returned Function InferredDescription qint64 addS64_int Performs signed 64-bit integer addition using SSE2 when available, detects signed overflow...",""],
["./appendix---trait-examples.html","Introduction &#62; Appendix","Enter topic text here.",""],
["./architectural-high-value.html","Introduction &#62; Appendix &#62; Function Index &#62; Architectural High-Value","Enter topic text here.",""],
["./architectural-overview---memor.html","Introduction &#62; Appendix &#62; Alpha &#62; Architectural Overview - Memory System","+----------------------------+                      &#124;    GlobalIRQController &#38;nbs...",""],
["./architecture-overview.html","Introduction &#62; Architecture Overview","1. Purpose of This Chapter  This chapter provides a top-level architectural description of the ASA EmulatR system. Its purpose is to explain ...",""],
["./architecture.html","Introduction &#62; Architecture","Architecture Navigation: Introduction Architecture ",""],
["./arithextender_helpers.html","Introduction &#62; Appendix &#62; Function Index &#62; CoreLib &#62; arithExtender_helpers","Returned Function Name Inferred Description void seedEmulatrRandom Initializes the legacy Qt random number generator (qsrand) for older Qt version...",""],
["./asa-emulatr_ini.html","Introduction &#62; Appendix &#62; Alpha &#62; Config &#62; ASA-EmulatR.ini","Enter topic text here.",""],
["./ast-delivery.html","Introduction &#62; Appendix &#62; Alpha &#62; Interrupts / IPL and AST &#62; AST Delivery","PAL_AST_USER handler:     1. Save context to process block     2. Vector to OS AST dispatcher     ↓ OS AST handler: ...",""],
["./automatic-generationgrains.html","Introduction &#62; Appendix &#62; Alpha &#62; Grains &#62; Automatic Generation","Python Script:  py grain_validator.py --tsv z:\\EmulatrAppUni\\grainfactoryLib\\grains\\GrainMaster.tsv --generate     --output z:\\Emul...",""],
["./axp_attributes_core.html","Introduction &#62; Appendix &#62; Function Index &#62; CoreLib &#62; Axp_Attributes_core","Returned / Defined Name Inferred Description macro AXP_COMPILER_MSVC Compile-time flag indicating MSVC compiler macro AXP_COMPILER_CLANG Compile-t...",""],
["./backup-gude-claude-todo.html","Introduction &#62; Appendix &#62; Backup Gude Claude TODO","# MEMORY ARCHITECTURE REFACTORING - IMPLEMENTATION GUIDE",""],
["./barriers.html","Introduction &#62; Appendix &#62; Alpha &#62; Cpu &#62; Barriers","Enter topic text here.",""],
["./bootstrap.html","Introduction &#62; Appendix &#62; Alpha &#62; Bootstrap","AlphaStation Emulator Console V1.0-0     ASA EmulatR (c) 2025 Timothy Peer / eNVy Systems, Inc.           &#38;nbsp...",""],
["./box-architecture.html","Introduction &#62; Appendix &#62; Alpha &#62; BOX Architecture","Silicon Box EmulatR Subsystem IBox IBox class + Pipeline fetch/decode stages + iGrain resolver Responsibilities:  Fetches PC Requests PC tran...",""],
["./box-orchestration.html","Introduction &#62; Appendix &#62; Alpha &#62; BOX Architecture &#62; Box Orchestration","Top-Level CPU Diagram  +---------------------------+ &#124;         AlphaCPU          &#124; &#124;------------...",""],
["./branch-prediction-alphapipeline.html","Introduction &#62; Appendix &#62; Alpha &#62; BOX Architecture &#62; Alpha PipeLine  &#62; Branch Prediction","Pipeline EX stage performs: execute instruction grain evaluate branch condition compute actual target call CBox::resolveBranch(...) if mispredict ...",""],
["./branch-prediction.html","Introduction &#62; Appendix &#62; Alpha &#62; Cpu &#62; Run Loop &#62; Branch Prediction","Models Supported:  Pattern History Table   Rules:  Is part of the control flow and is implemented in the CBox (control box)   ...",""],
["./cache-alpha.html","Introduction &#62; Appendix &#62; Alpha &#62; Cache","Enter topic text here.",""],
["./cbox.html","Introduction &#62; Appendix &#62; Alpha &#62; BOX Architecture &#62; CBox","Rules.  Owns Branch Prediction and  Branch Resolution Is truth source for prediction + correctness checking. Pipeline is truth source fo...",""],
...
["./chapter-14---execution-domains.html","Introduction &#62; Architecture Overview &#62; Chapter 14 – Execution Domains (&#8220;Boxes&#8221;)","The ASA EmulatR processor model is organized into six discrete execution domains, each implemented as a self-contained C++ class known as a \"...",""],
["./chapter-15---memory-system-imp.html","Introduction &#62; Architecture Overview &#62; Chapter 15 – Memory System Implementation Details","This chapter describes how memory is actually implemented inside ASA EmulatR, not merely how it behaves architecturally. Where earlier chapters de...",""],
["./chapter-16---device-model--dma.html","Introduction &#62; Architecture Overview &#62; Chapter 16 – Device Model &#38; DMA","This chapter describes the implementation of the ASA-EmulatR device subsystem: how devices are constructed, differentiated, routed through MMIO, t...",""],
["./chapter-17---tlb_-pte_-and-add.html","Introduction &#62; Architecture Overview &#62; Chapter 17 – Address Translation, TLB, and PTE","This chapter describes the implementation of address translation in ASA-EmulatR: how virtual addresses are converted to physical addresses, how TL...",""],
["./chapter-18---fault-dispatcher-.html","Introduction &#62; Architecture Overview &#62; Chapter 18 – Fault Dispatcher &#38; Precise Exceptions","This chapter describes the implementation of the ASA-EmulatR exception and fault delivery subsystem: how exceptional events are created, classifie...",""],
["./chapter-19---debugging_-tracin.html","Introduction &#62; Architecture Overview &#62; Chapter 19 – Interrupt Architecture &#38; IPI","This chapter describes the implementation of the ASA-EmulatR interrupt and inter-processor interrupt (IPI) subsystem: how interrupts are generated...",""],
["./chapter-1_.html","Introduction &#62; Architecture Overview &#62; Chapter 1 - System Overview","1.1 Purpose and Audience  ASA-EmulatR is a cycle-based, SMP-capable Alpha AXP processor emulator designed to faithfully reproduce the archite...",""],
["./chapter-1_2---design-goals-and.html","Introduction &#62; Architecture Overview &#62; Chapter 1 - System Overview &#62; 1.2 Design Goals and Guiding Principles","ASA-EmulatR is designed around five primary goals, listed in strict priority order. Every design decision in the emulator is evaluated against the...",""],
["./chapter-1_3---major-architectu.html","Introduction &#62; Architecture Overview &#62; Chapter 1 - System Overview &#62; 1.3 Target Architecture","ASA-EmulatR targets the following architectural profile:",""],
["./chapter-1_4---architectural-in.html","Introduction &#62; Architecture Overview &#62; Chapter 1 - System Overview &#62; 1.4 Major Architectural Layers","ASA-EmulatR is structured into six conceptual layers. Each layer has a clearly defined responsibility and communicates with adjacent layers throug...",""],
["./chapter-2---execution-model.html","Introduction &#62; Architecture Overview &#62; Chapter 2 - Execution Model","2.1 Cycle-Based Execution  ASA EmulatR operates on a cycle-based execution model where each iteration of the AlphaCPU run loop represents one...",""],
["./chapter-20---boot-sequence_-pa.html","Introduction &#62; Architecture Overview &#62; Chapter 20 – Boot Sequence, PAL, and SRM Integration","This chapter describes the implementation of the ASA-EmulatR Privileged Architecture Library (PAL) emulation layer and the boot sequence that brin...",""],
["./chapter-21---debugging_-tracin.html","Introduction &#62; Architecture Overview &#62; Chapter 21 – Debugging, Tracing, and Determinism","This chapter defines how ASA-EmulatR enables systematic debugging, high-fidelity tracing, and repeatable deterministic execution across single-CPU...",""],
["./chapter-22---testing_-validati.html","Introduction &#62; Architecture Overview &#62; Chapter 22 – Testing, Validation, and Architectural Compliance","This chapter defines the verification strategy for ASA-EmulatR. It explains how correctness is tested, how regressions are prevented, and how arch...",""],
["./chapter-2_1-purpose-of-this-ch.html","Introduction &#62; Architecture Overview &#62; Chapter 2 - Execution Model &#62; 2.1 Purpose of this Chapter","This chapter defines the execution model of ASA-EmulatR. It explains how AlphaCPU instances advance instructions, how cycles are defined and proce...",""],
["./chapter-2_10-summary.html","Introduction &#62; Architecture Overview &#62; Chapter 2 - Execution Model &#62; 2.10 Summary","The ASA-EmulatR execution model is: Cycle-based and deterministic — one iteration equals one hardware cycle Pipeline-driven with explicit serializ...",""],
["./chapter-2_2-cycle-base-executi.html","Introduction &#62; Architecture Overview &#62; Chapter 2 - Execution Model &#62; 2.2 Cycle-Base Execution Model","2.2.1 One Cycle per Iteration  ASA-EmulatR uses a cycle-based execution model: Each iteration of the AlphaCPU run loop represents one hardwar...",""],
...
["./chapter-2_6-speculation-policy.html","Introduction &#62; Architecture Overview &#62; Chapter 2 - Execution Model &#62; 2.6 Speculation Policy","ASA-EmulatR enforces a conservative but correct speculation model:",""],
...
["./chapter-3---pipeline-architect.html","Introduction &#62; Architecture Overview &#62; Chapter 3 - Pipeline Architecture","3.1 Purpose of This Chapter  This chapter defines the structure, responsibilities, and invariants of the AlphaPipeline in ASA EmulatR.  ...",""],
...
["./chapter-3_16-summary.html","Introduction &#62; Architecture Overview &#62; Chapter 3 - Pipeline Architecture &#62; 3.18 Summary","The AlphaPipeline in ASA-EmulatR is in-order, execute-centric, deterministic, precisely serialized, and SMP-safe by design.",""],
["./chapter-3_2-overview-of-the-pi.html","Introduction &#62; Architecture Overview &#62; Chapter 3 - Pipeline Architecture &#62; 3.2 Overview of the Pipeline Model","ASA-EmulatR implements a six-stage, in-order Alpha AXP pipeline optimized for clarity, correctness, and determinism. The pipeline is a single-issu...",""],
...
["./chapter-4_2-box-based-executio.html","Introduction &#62; Architecture Overview &#62; Chapter 4 - Functional Execution Domains (\"Boxes) &#62; 4.2 Box-Based Execution Model","4.2.1 Conceptual Model  In ASA-EmulatR, Boxes are not pipeline stages and not execution units in the scheduling sense. They are functional do...",""],
...
["./chapter-5_14-smp-consideration.html","Introduction &#62; Architecture Overview &#62; Chapter 5 - Memory System Architecture &#62; 5.14 SMP Considerations","In ASA-EmulatR SMP systems: All CPUs share GuestMemory — one global instance, one backing store, one PA routing table Each CPU has independent wri...",""],
["./chapter-5_15-summary.html","Introduction &#62; Architecture Overview &#62; Chapter 5 - Memory System Architecture &#62; 5.15 Summary","The ASA-EmulatR memory architecture mirrors Alpha AXP behavior while remaining debuggable, testable, and extensible.",""],
...
["./chapter-6_13-serialization-in-.html","Introduction &#62; Architecture Overview &#62; Chapter 6 - Serialization and Stall Model &#62; 6.13 Serialization in SMP Systems ","In ASA-EmulatR SMP configurations, serialization has cross-CPU implications:",""],
["./chapter-6_15-summary.html","Introduction &#62; Architecture Overview &#62; Chapter 6 - Serialization and Stall Model &#62; 6.14 Summary","The ASA-EmulatR serialization model preserves Alpha AXP&#39;s explicit ordering semantics while providing clear, debuggable barrier mechanics.",""],
...
["./chapter-7_13-summary.html","Introduction &#62; Architecture Overview &#62; Chapter 7 - Exceptions, Faults, and Interrupts &#62; 7.13 Summary","The ASA-EmulatR exception system provides precise, deterministic exception handling that matches Alpha AXP architectural requirements.",""],
...
["./chapter-8_10-enforcing-the-pri.html","Introduction &#62; Architecture Overview &#62; Chapter 8 - PAL and Privileged Boundary &#62; 8.10 Enforcing the Privileged Boundary","The privileged boundary in ASA-EmulatR is absolute. Execution state is either inside PAL mode or outside PAL mode — there is no intermediate state...",""],
...
["./chapter-9_2-smp-design-philoso.html","Introduction &#62; Architecture Overview &#62; Chapter 9 - SMP Architecture &#62; 9.2 SMP Design Philosophy","ASA-EmulatR follows five core SMP principles:  True symmetry — all CPUs are peers. There is no \"master\" CPU. Any CPU can boot, hand...",""],
...
["./chapter-style-guide-and-cross-.html","Introduction &#62; Chapter Style Guide and cross ref. Analysis.md","# ASA-EmulatR Documentation — Style Guide &#38;amp; Cross-Reference Analysis",""],
...
["./check_circular_py-appendix.html","Introduction &#62; Appendix &#62; Python Scripts &#62; check_circular.py","Usage:      python check_circular_includes.py /path/to/project     python check_circular_includes.py /path/to/project --v...",""],
["./commands.html","Introduction &#62; Appendix &#62; Alpha &#62; Bootstrap &#62; Commands","# Edit ASAEmulatr.ini (add new disk, modify network settings)",""],
["./compiler-options.html","Introduction &#62; Compiler Options","AXP_DEBUG_ENABLED  AXP_WARN_ENABLED  AXP_TRACE_ENABLED  AXP_CRITICAL_ENABLED",""],
["./config.html","Introduction &#62; Appendix &#62; Alpha &#62; Config","Element Description",""],
["./controllers.html","Introduction &#62; Appendix &#62; Alpha &#62; Controllers","Element Description",""],
["./conventions.html","Introduction &#62; Appendix &#62; Conventions","SEXT SEXT(x) X is sign-extended to the required size. RIGHT_SHIFT(x,y) Logical right shift of first operand by the second operand. Y is an unsigne...",""],
["./core-processor-implementation.html","Introduction &#62; Core Processor Implementation","Enter topic text here.",""],
["./corelib.html","Introduction &#62; Appendix &#62; Function Index &#62; CoreLib","Function Description Usage  addQ Signed quadword addition with overflow tracking Integer execution, EBox  advancePC Advance PC by fixed ...",""],
["./cow-support-alphaspammanager.html","Introduction &#62; Appendix &#62; Alpha &#62; Managers &#62; Alpha_SPAM_Manager &#62; COW Support","Copy-on-Write (COW) is a memory management technique used to efficiently handle duplicate copies of memory pages. When a process forks, instead of...",""],
["./cpu.html","Introduction &#62; Appendix &#62; Alpha &#62; Cpu","Summary Table Pointer/RefPurposeOwnership RegisterBank*Integer registersAlphaCPU (per-vCPU) FpRegisterBank*FP registersAlphaCPU (per-vCPU) VectorR...",""],
["./cpucorelib.html","Introduction &#62; Appendix &#62; Function Index &#62; cpuCoreLib","Enter topic text here.",""],
["./cpustateiprinterface-taskable2.html","Introduction &#62; Appendix &#62; Taskables &#62; CPUStateIPRInterface taskables","Here’s the matching **System Side-Effect Checklist** for `CPUStateIPRInterface`, in the same style as the one we just did for `EV6_Hooks`.",""],
["./cpustateiprinterface-taskables.html","Introduction &#62; Appendix &#62; Taskables &#62; CPUStateIPRInterface-Taskables","CPUStateIPRInterface-Taskables Navigation: Introduction Appendix Taskables CPUStateIPRInterface-Taskables ",""],
["./cserve-environment-variable-im.html","Introduction &#62; Appendix &#62; Keep &#62; CSERVE Environment Variable Implementation","ASAEmulatR CSERVE Environment Variable Implementation",""],
["./cserve-selector-table-based-on.html","Introduction &#62; Architecture Overview &#62; CSERVE selector table based on SRM behavior","Enter topic text here.",""],
["./currentcputls.html","Introduction &#62; Appendix &#62; Function Index &#62; CoreLib &#62; CurrentCpuTls","Returned Function Name Inferred Description void set(quint16 id) Sets the current CPU identifier in thread-local storage for the executing host th...",""],
["./debugging-sessoins.html","Introduction &#62; Appendix &#62; Debugging Sessoins","// PC and instruction pc                              // Current progra...",""],
["./debugging_-tracing_-and-valida.html","Introduction &#62; Debugging, Tracing, and Validation","Logging system  Trace hooks  Determinism guarantees  Invariant validation  Test strategy",""],
["./decodedinstruction.html","Introduction &#62; DecodedInstruction","// ============================================================================",""],
["./define_helpers.html","Introduction &#62; Appendix &#62; Function Index &#62; CoreLib &#62; define_helpers","Returned Function / Macro Name Inferred Description void MEM_BARRIER Full memory fence ensuring sequential consistency across all threads. void ME...",""],
["./dependency-requirements.html","Introduction &#62; Appendix &#62; Dependency Requirements","Coverage Map    IRQController  ReservationManager  SafeMemory  MMIOManager  AlphaCPU  DeviceManager  Syste...",""],
["./device-subsystem.html","Introduction &#62; Device Subsystem","MMIOManager  DeviceManager  IRQ routing  DMA modeling  Specific device implementations",""],
["./devicemanagerdependency.html","Introduction &#62; Appendix &#62; Dependency Requirements &#62; DeviceManager","DMA Coherence DEV-REQ-001: Any DMA write must call SafeMemory → ReservationManager. DEV-REQ-002: Devices must NOT bypass SafeMemory unless they ex...",""],
["./directory-structure.html","Introduction &#62; Directory Structure","**DIRECTORY STRUCTURE:** ``` bin/ ├── debug/ │   ├── emulator.exe          ← Executable │   ├── config/ │ &#38;nbsp...",""],
["./discussions.html","Introduction &#62; Appendix &#62; Alpha &#62; Discussions","Enter topic text here.",""],
["./dual-issue-hazard-scoreboard.html","Introduction &#62; Appendix &#62; Alpha &#62; BOX Architecture &#62; Alpha PipeLine  &#62; ScoreBoard Hazards &#62; Dual Issue Hazard","Case Dual Issue:  Slot(2): A Slot(3): B  Case: A and B both write the same register (WAW across pair)  A: ADDQ R1, R2, R3 B: SUBQ R...",""],
["./dumpbin.html","Introduction &#62; Appendix &#62; DumpBin","Z:\\EmulatR\\x64\\Debug&#62;dumpbin /exports Z:\\EmulatR\\x64\\Debug\\CoreLib.lib",""],
["./ebox.html","Introduction &#62; Appendix &#62; Alpha &#62; BOX Architecture &#62; EBox","Integer Instruction Grains Processing.  Responsible for all integer operations (grain Op/Func), and local Int Register Bank.  Grain Exec...",""],
["./emulatormanger-app.html","Introduction &#62; Appendix &#62; Dependency Requirements &#62; EmulatorManger App","Critical Initialization Order:   1. globalSettings().loadFromConfig(json); 2. globalIPRBank().init(numCpus); 3. globalHWPCBBank().init(numCpu...",""],
["./emulatr-initialization-.html","Introduction &#62; EmulatR Initialization ","# EmulatR Initialization Architecture  **Version:** 1.0   **Date:** December 2025   **Pattern:** QScopedPointer + Meyer&#39;s Singleton...",""],
["./emulatr-logging.html","Introduction &#62; Appendix &#62; EmulatR Logging","# ASA Emulator Logging System  **Version:** 2.0   **Date:** December 2025   **Architecture:** Queue-based async logging with multip...",""],
["./endianness-rules.html","Introduction &#62; Endianness Rules","Alpha is ALWAYS little-endian. There is no big-endian mode. Any big-endian data must be explicitly byte-swapped by software.",""],
["./ev4.html","Introduction &#62; Appendix &#62; Alpha &#62; IPR &#62; EV4","Element Description",""],
["./ev5.html","Introduction &#62; Appendix &#62; Alpha &#62; IPR &#62; EV5","Element Description",""],
["./ev6.html","Introduction &#62; Appendix &#62; Alpha &#62; IPR &#62; EV6","Constr Mnuemonic BOX Decimal ScoreBoard Bit Access MT/MF Issued from Pipe   #define ITB_TAG0; ITB_TAG I 0 6 WO 0L   #define ITB_PTE1; IT...",""],
["./ev6ptecache.html","Introduction &#62; Appendix &#62; Dependency Requirements &#62; Ev6PteCache","How EV6 uses two DTB banks (&#8220;Pipe 0&#8221; and &#8220;Pipe 1&#8221;)",""],
["./ev6_hooks_taskables.html","Introduction &#62; Appendix &#62; Taskables &#62; EV6_Hooks_Taskables","Absolutely — here is a **fully structured, facility-level system-side-effect checklist** for **EV6_Hooks**, matching the exact format you approved...",""],
["./exception--faults.html","Introduction &#62; Appendix &#62; Alpha &#62; Exception &#38; Faults","instructions implement a FP exception latch layer.",""],
["./exception-handling-fbox.html","Introduction &#62; Appendix &#62; Alpha &#62; BOX Architecture &#62; FBox &#62; Exception handling","Exceptions generated by the Fbox are recorded in two places:",""],
["./exceptionfactory_inl.html","Introduction &#62; Appendix &#62; Function Index &#62; ExceptionLib &#62; ExceptionFactory_inl","Function Description Usage PendingEvent makeITBMissEvent @brief Create ITB miss event  * @param cpuId CPU that raised exception  * @para...",""],
["./exceptionlib.html","Introduction &#62; Appendix &#62; Function Index &#62; ExceptionLib","ExceptionLib Navigation: Introduction Appendix Function Index ExceptionLib ",""],
["./exceptionmapping_inl.html","Introduction &#62; Appendix &#62; Function Index &#62; ExceptionLib &#62; ExceptionMapping_inl","SINGLE SOURCE OF TRUTH for ExceptionClass -&#62; PalVectorId resolution.",""],
["./exceptionpreparation_inl.html","Introduction &#62; Appendix &#62; Function Index &#62; ExceptionLib &#62; ExceptionPreparation_inl","Returns Function Description / Usage void preparePendingEventForDelivery Prepare pending event for delivery to PAL This is the SINGLE preparation ...",""],
["./exceptions.html","Introduction &#62; Appendix &#62; Alpha &#62; Exceptions Vector Map","Header:  crtLib::PAL_Vector_Controller.h     PalVectorid Offset Description Side effects RESET 0X0000   MCHK 0X0080 Machine ch...",""],
["./exceptionstateupdate_inl.html","Introduction &#62; Appendix &#62; Function Index &#62; ExceptionLib &#62; ExceptionStateUpdate_inl","Returns Function Description / Usage void updateExceptionSummary Update EXC_SUM register based on exception type EXC_SUM encodes exception-specifi...",""],
["./exc_sum_helpers.html","Introduction &#62; Appendix &#62; Function Index &#62; CoreLib &#62; IPR Hive &#62; EXC_SUM_helpers","Returns Function Inferred Usage bool excSumHasSWC Test EXC_SUM for Software Completion Possible (SWC) arithmetic-trap bit bool excSumHasINV Test E...",""],
["./executeexcb-barriers.html","Introduction &#62; Appendix &#62; Alpha &#62; Cpu &#62; Barriers &#62; executeEXCB","// ============================================================================",""],
["./executewmb-barriers.html","Introduction &#62; Appendix &#62; Alpha &#62; Cpu &#62; Barriers &#62; executeMB_WMB","// ============================================================================",""],
["./export-symbols-appendix.html","Introduction &#62; Appendix &#62; Export Symbols","===============================================================================",""],
["./fbox.html","Introduction &#62; Appendix &#62; Alpha &#62; BOX Architecture &#62; FBox","Integer Instruction Grains Processing.  Responsible for all Floating-Point operations (grain Op/Func), and local FP Register Bank.  Grai...",""],
["./flow-chart-runloop.html","Introduction &#62; Flow-Chart-RunLoop","================================================================================",""],
["./fpcr-fbox-registers.html","Introduction &#62; Appendix &#62; Alpha &#62; BOX Architecture &#62; FBox &#62; Registers &#62; FPCR ","Name Offset Implementation",""],
["./function-flow-branchpredition.html","Introduction &#62; Appendix &#62; Alpha &#62; Cpu &#62; Run Loop &#62; Function Flow ","Predictive Fetch (IBox)  IBox.fetchNext():     pc = ctx.getPC()     predictedTaken = cbox.predictBranch_IBox(pc, disp) &#38;n...",""],
["./generate_all_grains_py-appendix.html","Introduction &#62; Appendix &#62; Python Scripts &#62; generate_all_grains.py","Script Command Line: py generate_all_grains.py Z:\\EmulatRAppUni\\grainFactoryLib\\GrainMaster.tsv Z:\\EmulatRAppUni",""],
["./generate_cmakelists_py-appendix.html","Introduction &#62; Appendix &#62; Python Scripts &#62; generate_cmakelists.py","Command Line: py generate_cmakelists.py Z:\\EmulatRAppUni\\",""],
["./ghwpcbbank_globalstatics.html","Introduction &#62; Appendix &#62; Meyer&#39;s Singleton &#62; globalHWPCBManager","inline HwpcbManager&#38; globalHwpcbManager() {     static HwpcbManager instance;     return instance; }    Host the...",""],
["./giprbank-(iprbank)_qglobalstatics.html","Introduction &#62; Appendix &#62; Meyer&#39;s Singleton &#62; globalIprRegistry","inline IprDescriptorRegistry&#38; globalIprRegistry();",""],
["./girqcontroller_qglobalstatics.html","Introduction &#62; Appendix &#62; Meyer&#39;s Singleton &#62; globalIRQController","Interrupt Controller (IPL arbitrator and delivery). ",""],
["./globalev6ptecache.html","Introduction &#62; Appendix &#62; Meyer&#39;s Singleton &#62; globalEv6PteCache","Implementation:  Put the SPAMManager singleton into Layer 0 (silicon). ✔ Initialize it once at emulator boot through a static initialize() me...",""],
["./globalipr_hot_cold.html","Introduction &#62; Appendix &#62; Function Index &#62; CoreLib &#62; globalIPR_hot_cold","Returned Type Function Name Inferred Description IPRStorage_Hot&#38; Reference globalIPRHot Returns a reference to the per-CPU hot IPR storage use...",""],
["./globalreservationmanager.html","Introduction &#62; Appendix &#62; Meyer&#39;s Singleton &#62; globalReservationManager","inline ReservationManager&#38; globalReservationManager()",""],
["./global_emulatorsettings.html","Introduction &#62; Appendix &#62; Meyer&#39;s Singleton &#62; Global_EmulatorSettings","1. Load config (once): Global_EmulatorSettings().loadFromIni(\"emulator.ini\");  2. Access anywhere: int mem = Global_EmulatorSetting...",""],
["./global_hwpcbbank_interface.html","Introduction &#62; Appendix &#62; Function Index &#62; CoreLib &#62; Global_HWPCBBank_Interface","Returned Function Name Inferred Description HWPCBBank HWPCBBank(CPUIdType cpuCnt) Constructs a per-CPU HWPCB bank with multiple context slots per ...",""],
["./grain-routing---pal.html","Introduction &#62; Appendix &#62; Alpha &#62; PAL &#62; Grain Routing - PAL","Key architectural rule (reinforced) ----------------------------------- - PAL grains should NOT force a return to User mode on exit. - PAL-mode is...",""],
["./grainmaster_tsv-appendix.html","Introduction &#62; Appendix &#62; GrainMaster.tsv","OpcodeFunctionMnemonicDescriptionTypeBox 0x000x0000CALL_PALCALL_PALPALcodePalBox 0x000x0000HALTHALTPALcodePalBox 0x000x0001RESTARTRESTARTPALcodePa...",""],
["./grains_.html","Introduction &#62; Appendix &#62; Alpha &#62; Grains","Lock-free with versioning! This is production-grade:",""],
["./guestmemory-srm-integration.html","Introduction &#62; Appendix &#62; Keep &#62; GuestMemory SRM Integration","# GuestMemory SRM Integration - Complete Guide  ## Summary  You asked how to add **SRM as a routing target** in your existing GuestMemor...",""],
["./helper-functions.html","Introduction &#62; Appendix &#62; Helper Functions","Helper Location AccessMasks (RE: WR: EX) - hasPermission - canUserRead/Write/Exec - canKernelRead/Write genlib::define_static_const.h quint8 - Bit...",""],
["./hwpcb-.html","Introduction &#62; Appendix &#62; Alpha &#62; Hardware Privileged Context Block (HWPCB)","HWPCB (Hardware Privileged Context Block) - Field List",""],
["./hwpcb_taskables.html","Introduction &#62; Appendix &#62; Taskables &#62; HWPCB_Taskables","Absolutely — let me give you a clear, correct answer **without needing a file search**, because your question is conceptual and the code context i...",""],
["./i-cache-cache.html","Introduction &#62; Appendix &#62; Alpha &#62; Cache &#62; I-Cache","Primary Responsibility.  Rules. Only I-cache invalidation events should clear predictor state.   Handles.",""],
["./ibox.html","Introduction &#62; Appendix &#62; Alpha &#62; BOX Architecture &#62; IBox","IBox performs decode + issue, NOT execution  Dispatches instruction grain units to the associated BOX; does not &#8220;schedule&#8221; globally.  Br...",""],
["./implementation-alpha_spam_manager.html","Introduction &#62; Appendix &#62; Alpha &#62; Managers &#62; Alpha_SPAM_Manager &#62; Implementation","# TLB Integration To-Do List  CPUStateIPRInterface.h - [x] Add per-CPU pointers to ITB and DTB managers (`itbManager` and `dtbManager`) - [x]...",""],
["./implementation.html","Introduction &#62; Appendix &#62; Alpha &#62; SCSI &#62; Implementation","Got it, thanks for the correction – I’ll ignore the &#8220;coreLib&#8221; idea for SCSI and lay out a clean header structure and placement for everything SCSI...",""],
["./integration-steps.html","Introduction &#62; Appendix &#62; Alpha &#62; Integration Steps","Phase 0 – Freeze the new &#8220;truths&#8221;  Before touching call-sites, treat these as the canonical interfaces:  Pipeline &#38; slots – AlphaPip...",""],
["./interrupt---methodology--conve.html","Introduction &#62; Appendix &#62; Alpha &#62; Interrupts / IPL and AST &#62; Interrupt - Methodology &#38; Convention","IPR-ORDER-OF-OPERATIONS.TXT  (ASCII)  ## Goal  Define a concrete, stepwise \"state transition script\" for an EV6 (21264) i...",""],
["./interrupts-_-ipl-and-ast.html","Introduction &#62; Appendix &#62; Alpha &#62; Interrupts / IPL and AST","Enter topic text here.",""],
["./introduction.html","Introduction","EmulatR Alpha/Tru64 emulation.     Architecture Overview What kind of machine is this? It defines what must be true, not how it’s implem...",""],
["./ipi---inter-processor-interrup.html","Introduction &#62; Appendix &#62; Alpha &#62; Interrupts / IPL and AST &#62; IPI - Inter Processor Interrupt Handling","CPU 0 executes MTPR IPIR (target=CPU 1)     ↓ 1. Queue IPI message in CPU 1&#39;s IPIQueue 2. Request interrupt via IRQ controller at IPI_IP...",""],
["./ipr-cache-optimization---quick.html","Introduction &#62; Appendix &#62; Keep &#62; IPR Cache Optimization - Quick Reference","Architecture Overview  ┌─────────────────────────────────────────────────────────────┐ │  PerCpuIPRState (per CPU, cache-line isolated) ...",""],
["./ipr-hive.html","Introduction &#62; Appendix &#62; Function Index &#62; CoreLib &#62; IPR Hive","Internal Processor Register - Relevance Classification.",""],
["./ipr-storage.html","Introduction &#62; Appendix &#62; Function Index &#62; CoreLib &#62; IPR Hive &#62; IPR Storage","IPR Storage Navigation: Introduction Appendix Function Index CoreLib IPR Hive IPR Storage ",""],
["./ipr.html","Introduction &#62; Appendix &#62; Alpha &#62; IPR","Element Description",""],
["./iprstorage.html","Introduction &#62; Appendix &#62; Dependency Requirements &#62; IPRStorage","ReservationManager Trap / Exception / AST / Mode change ReservationManager IPR writes that alter process context  Examples: MTPR PTBR MTPR AS...",""],
["./iprstorage_ibox.html","Introduction &#62; Appendix &#62; Function Index &#62; CoreLib &#62; IPR Hive &#62; IprStorage_IBox","Returned Function Name Inferred Description quint16 ICtlRegister::vptb_sext() Returns the sign-extension bits for the virtual page table base (VPT...",""],
["./ipr_getactivesp_inl.html","Introduction &#62; Appendix &#62; Function Index &#62; CoreLib &#62; IPR Hive &#62; Ipr_getActiveSP_inl","Returned Function Name Inferred Description quint64 getActiveSP Returns the currently active stack pointer (SP) for the calling CPU based on the a...",""],
["./ipr_ic_flush_inl.html","Introduction &#62; Appendix &#62; Function Index &#62; CoreLib &#62; IPR Hive &#62; IPR_IC_FLUSH_inl","Returned Function Name Inferred Description void writeIPR_ICTL Handles writes to the I_CTL IPR: masks read-only fields, preserves architectural bi...",""],
["./irqcontroller-controllers.html","Introduction &#62; Appendix &#62; Alpha &#62; Controllers &#62; IRQController","# IRQController Integration Guide  **Version:** 1.0   **Date:** December 2025   **Author:** Timothy Peer / eNVy Systems, Inc. &#38;nbsp...",""],
["./irqcontrollerdependencies.html","Introduction &#62; Appendix &#62; Dependency Requirements &#62; IRQController","IRQ Routing Requirements IRQ-REQ-001: Provide per-vector routing policy (FIXED_CPU, ROUND_ROBIN, LOWEST_IPL, BROADCAST). IRQ-REQ-002: Store per-ve...",""],
["./keep.html","Introduction &#62; Appendix &#62; Keep","Enter topic text here.",""],
["./lifecycle-grain.html","Introduction &#62; Appendix &#62; Alpha &#62; Grains &#62; Lifecycle ","A characterization aligned with EV6/ASA semantics and the current EmulatR design.",""],
["./load---silos-.html","Introduction &#62; Appendix &#62; Alpha &#62; Load - Silos ","The Abox contains a memory reference pipeline that can accept a new load or store instruction every cycle until a Dcache fill is required. Since t...",""],
["./logging.html","Introduction &#62; Appendix &#62; Alpha &#62; Logging","┌────────────────────────── LogReader Application ─────────────────────────┐",""],
["./makearithmeticfaultinfo_inl.html","Introduction &#62; Appendix &#62; Function Index &#62; CoreLib &#62; makeArithmeticFaultInfo_inl","Returned Function Name Inferred Description MemoryFaultInfo makeArithmeticFaultInfo Constructs and returns a fully populated MemoryFaultInfo struc...",""],
["./managers.html","Introduction &#62; Appendix &#62; Alpha &#62; Managers","Element Description",""],
["./mbox.html","Introduction &#62; Appendix &#62; Alpha &#62; BOX Architecture &#62; MBox","Below is a **clean, grain-scoped specification** for **MBox responsibilities**, deliberately restricted to **what instruction grains executed in t...",""],
["./memory--synchronization-intern.html","Introduction &#62; Memory &#38; Synchronization Internals","SafeMemory internals  GuestMemory routing  ReservationManager  Cache &#38; coherency modeling",""],
["./memory-mapvisual.html","Introduction &#62; Memory MapVisual","# CANONICAL PA ROUTING TABLE (Option A) # ASA Emulator - Single Source of Truth for Physical Address Mapping # Date: 2025-01-27  ## DESIGN PR...",""],
["./minimal-pal-checklist.html","Introduction &#62; Architecture Overview &#62; Minimal Pal checklist","Excellent instinct — this is exactly the *right* pivot point.",""],
["./mmio.html","Introduction &#62; Appendix &#62; Alpha &#62; MMIO","Element Description",""],
["./mmiomanagerdependency.html","Introduction &#62; Appendix &#62; Dependency Requirements &#62; MMIOManager","MMIO Writes MMIO-REQ-001: Device register writes do not break reservations. MMIO-REQ-002: Device DMA writes into RAM must call ReservationManager ...",""],
...
["./next-steps-to-srm-.html","Introduction &#62; Architecture Overview &#62; Next Steps to SRM &#62;&#62;&#62;","You’re exactly right: at this point, what’s missing is **not more architecture prose**, but a **concrete, deterministic operational sequence** tha...",""],
["./next-steps.html","Introduction &#62; Architecture Overview &#62; Next Steps","You’re feeling exactly the *right* thing right now �� — and this is actually a **very good sign**.",""],
["./oniplchanged_inl.html","Introduction &#62; Appendix &#62; Function Index &#62; CoreLib &#62; onIPLChanged_inl","Returned Function Name Inferred Description void onIPLChanged Notifies the global IRQ controller that a CPU’s Interrupt Priority Level (IPL) has c...",""],
["./osf.html","Introduction &#62; Appendix &#62; Alpha &#62; Platform &#62; OSF","Element Description",""],
["./pagetable(pte)subsystem.html","Introduction &#62; Appendix &#62; Alpha PTE Subsystem","Overview  Update:   Because the SPAM/TLB subsystem acts like a 4-D associative array: CPU Realm SizeClass BucketIndex  -------- The...",""],
["./pal-exception-vectoring.html","Introduction &#62; Appendix &#62; Alpha &#62; PAL &#62; PAL Exception Vectoring","// ============================================================================",""],
["./pal-module.html","Introduction &#62; Appendix &#62; Dependency Requirements &#62; PAL Module","COW is triggered by a write-fault → PAL → kernel trap handler",""],
["./pal-vector-table-(gpalvectorta.html","Introduction &#62; Appendix &#62; Meyer&#39;s Singleton &#62; globalPalVectorTable","PAL vector layout is static architecture data:  Vector index → handler function / grain / metadata.  All CPUs share the same PAL table; ...",""],
["./pal.html","Introduction &#62; Appendix &#62; Alpha &#62; PAL","PAL executes in MBox  PALcode does things such as:  Change privileged registers (IPRs)  Manipulate the HWPCB (context block)  ...",""],
["./pctx_helpers.html","Introduction &#62; Appendix &#62; Function Index &#62; CoreLib &#62; IPR Hive &#62; PCTX_helpers","Function / Helper Name Inferred Description decodePCTX_ASN Extracts the Address Space Number (ASN) field from a raw PCTX value decodePCTX_ASTER Ex...",""],
["./permissions--va-translation.html","Introduction &#62; Appendix &#62; Function Index &#62; Architectural High-Value &#62; Permissions &#38; VA Translation","CATEGORY: Memory Policy STABILITY: Architectural (High) CONSUMERS: MMU, TLB, ACV, PAL  Reference: permissions_helper_inl",""],
["./permissions_helper_inl.html","Introduction &#62; Appendix &#62; Function Index &#62; CoreLib &#62; permissions_helper_inl","Function Name Inferred Description pageShift Returns the bit shift corresponding to an Alpha page size code (used to compute page alignment and in...",""],
["./platform.html","Introduction &#62; Appendix &#62; Alpha &#62; Platform","Element Description",""],
["./processor-status-(ps)-helpers.html","Introduction &#62; Appendix &#62; Function Index &#62; Architectural High-Value &#62; Processor Status (PS) Helpers","Processor Status (PS) Helpers  Architectural Inference Summary  Layer: Core / Architectural State Stability: Architectural (High) Scope:...",""],
["./ps_helpers_inl.html","Introduction &#62; Appendix &#62; Function Index &#62; CoreLib &#62; PS_helpers_inl","Function Returns Description extractIPLFromPS quint8 Extracts the Interrupt Priority Level (IPL) field from PS insertIPLIntoPS quint64 Inserts a n...",""],
["./pte.html","Introduction &#62; Appendix &#62; Alpha &#62; PTE","# SMP-Coherent PTE Interface - Integration Guide  ## Overview  This interface provides **per-CPU SPAM caches** with **SMP coherence** fo...",""],
["./pte_core.html","Introduction &#62; Appendix &#62; Function Index &#62; CoreLib &#62; pte_core","Purpose: Low-level, header-only utilities used by the Alpha AXP MMU, TLB, and page table subsystems. Designed to be constexpr-friendly, pipeline-a...",""],
["./pupose-of-this-chapterchapter2.html","Introduction &#62; Architecture Overview &#62; Chapter 3 - Pipeline Architecture &#62; 3.1 Pupose of this Chapter","This chapter defines the structure, responsibilities, and invariants of the AlphaPipeline in ASA-EmulatR. It explains how instructions flow throug...",""],
["./purpose-and-audiencechapter1.html","Introduction &#62; Architecture Overview &#62; Chapter 1 - System Overview &#62; 1.1 Purpose and Audience","This chapter provides a top-level architectural description of the ASA-EmulatR Alpha AXP emulator. It defines the major subsystems, their responsi...",""],
["./python-scripts.html","Introduction &#62; Appendix &#62; Python Scripts","Enter topic text here.",""],
["./qglobalstatics_appendix.html","Introduction &#62; Appendix &#62; Meyer&#39;s Singleton","Global Statics Description globalHWPCBManager  globalIprRegistry  globalPalVectorTable  globalIRQController  globalReservation...",""],
["./qtrandomcompat.html","Introduction &#62; Appendix &#62; Function Index &#62; CoreLib &#62; QtRandomCompat","Determinism Note:  Uses host-provided randomness (Qt RNG or qrand). Results are not replay-safe unless externally seeded or overridden. Any s...",""],
["./r31-profiling.html","Introduction &#62; Appendix &#62; Alpha &#62; IPR &#62; R31 Usage &#62; R31 Profiling","// ============================================================================",""],
["./r31-usage.html","Introduction &#62; Appendix &#62; Alpha &#62; IPR &#62; R31 Usage","R31_USAGE_AND_WRITEBACK_SPEC.txt - (ASCII, UTF-8 noBOM)",""],
["./raw-hazard---load-followed-alu.html","Introduction &#62; Appendix &#62; Alpha &#62; BOX Architecture &#62; Alpha PipeLine  &#62; ScoreBoard Hazards &#62; RAW Hazard - Load Followed ALU","Load followed by dependent ALU  Scenario:  A: LDQ R1, 0(R2) B: ADDQ R3, R1, R4 (B needs R1 written by A)  Goal: B must not read R1 ...",""],
["./registers-ebox.html","Introduction &#62; Appendix &#62; Alpha &#62; BOX Architecture &#62; EBox &#62; Registers","Name  Offsets Implementation CC   CC_CTL   VA_FORM   VA_CTL   VA",""],
["./registers-fbox.html","Introduction &#62; Appendix &#62; Alpha &#62; BOX Architecture &#62; FBox &#62; Registers","Registers Navigation: Introduction Appendix Alpha BOX Architecture FBox Registers ",""],
["./registers-ibox.html","Introduction &#62; Appendix &#62; Alpha &#62; BOX Architecture &#62; IBox &#62; Registers","Name  Offsets Implementation EXC_SUM // EXC_SUM Register Layout (EV6): // Bits [63:48] - SEXT(SET_IOV)  Sign extension of SET_IOV bit //...",""],
["./registers-mbox.html","Introduction &#62; Appendix &#62; Alpha &#62; BOX Architecture &#62; MBox &#62; Registers","Name Offset Implementation DTB_TAG0   DTB_TAG1   DTB_PTE0   DTB_PTE1   DTB_ALTMODE   DTB_IAP   DTB_IA   MM_STAT...",""],
["./registers.html","Introduction &#62; Appendix &#62; Alpha &#62; BOX Architecture &#62; CBox &#62; Registers","CBox operations participate in the architectural order of operations as serialized, atomic control commits; the MSB-first 6-bit shifting is purely...",""],
["./reservationmanager-appendix.html","Introduction &#62; Appendix &#62; ReservationManager","Internal Operations (performed by ReservationManager)",""],
["./reservationmanager.html","Introduction &#62; Appendix &#62; Function Index &#62; cpuCoreLib &#62; ReservationManager","Keyword Description Consumed By ReservationManager Central LL/SC reservation controller AlphaCPU, SafeMemory, PAL handlers Reservation Per-CPU res...",""],
["./reservationmanagerdependency.html","Introduction &#62; Appendix &#62; Dependency Requirements &#62; ReservationManager","Structural Requirements RM-REQ-001: Provide per-CPU reservation tracking for LL/SC instructions. RM-REQ-002:  Track the reserved physical add...",""],
["./resolution-interface.html","Introduction &#62; Appendix &#62; Alpha &#62; Cpu &#62; Run Loop &#62; Branch Resolution","Called From:  Alpha PipeLine  Consumed by:  BranchResolutionResult resolveBranch(     quint64 pc,     bool predicted...",""],
["./run-loop-nested-stack.html","Introduction &#62; Architecture Overview &#62; Run Loop Nested Stack","=============================================================================",""],
["./run-loop.html","Introduction &#62; Appendix &#62; Alpha &#62; Cpu &#62; Run Loop","Fault &#38; PAL Responsibility  FaultDispatcher   └── detects conditions   └── builds PendingEvent   └── exposes PendingEvent ...",""],
["./safememory---ram.html","Introduction &#62; Appendix &#62; Alpha &#62; Managers &#62; SafeMemory - RAM","1. Scope and Responsibilities  **SafeMemory’s role**  * Provide a **safe, bounded, byte-addressable backing store** for guest *physical*...",""],
["./safememorydependency.html","Introduction &#62; Appendix &#62; Dependency Requirements &#62; SafeMemory","Coherence with ReservationManager SM-REQ-001: On any physical write to RAM, SafeMemory must call resConflictWrite(). SM-REQ-002: Device-backed mem...",""],
["./scoreboard-hazards.html","Introduction &#62; Appendix &#62; Alpha &#62; BOX Architecture &#62; Alpha PipeLine  &#62; ScoreBoard Hazards","The scoreboard’s purpose is to detect instruction hazards:",""],
["./scsi.html","Introduction &#62; Appendix &#62; Alpha &#62; SCSI","Element Description                   deviceLib/     ISCSIDevice.h     SCSIDisk.h...",""],
["./shard-scaffolding-ownership.html","Introduction &#62; Appendix &#62; Sharding Mechanism - PTE - SPAMShardManager &#62; Shard Scaffolding Ownership","Class Ownership and Invalidation Coordination in SMP",""],
["./sharding-mechanism---pte---spa.html","Introduction &#62; Appendix &#62; Sharding Mechanism - PTE - SPAMShardManager","What is Sharding? ----------------- Sharding is the process of partitioning a large set of cache (TLB/PTE) entries into smaller, semi-independent ...",""],
["./spam---policies.html","Introduction &#62; Appendix &#62; Alpha PTE Subsystem &#62; Cache Replacement and Invalidation Policies","Overview -------- The cache replacement and invalidation policies are critical components of the PTE subsystem&#39;s cache management strategy. These ...",""],
["./spam-shard-support.html","Introduction &#62; Appendix &#62; Taskables &#62; SPAM Shard Support","You’re very welcome �� And nice, this is exactly the right point to carve in the layering and stop letting templates leak all over the place.",""],
["./specation-pipeline.html","Introduction &#62; Appendix &#62; Alpha &#62; BOX Architecture &#62; Alpha PipeLine  &#62; Specation ","===============================================================================",""],
["./srm-load.html","Introduction &#62; Appendix &#62; Keep &#62; SRM load","╔══════════════════════════════════════════════════════════════════╗",""],
["./stall-conditions.html","Introduction &#62; Appendix &#62; Alpha &#62; Cpu &#62; Run Loop &#62; Stall Conditions","Q3: \"Two cases for stalling: Fault handling and pipeline stall (PalMode)\"",""],
["./systems-integration.html","Introduction &#62; Appendix &#62; Dependency Requirements &#62; Systems Integration","Integrate via EmulatR  SYS-REQ-001: EmulatorManager MUST construct and own: ReservationManager, IRQController, SafeMemory, MMIOManager SYS-RE...",""],
["./taskables.html","Introduction &#62; Appendix &#62; Taskables","Enter topic text here.",""],
["./tier.html","Introduction &#62; Appendix &#62; Alpha &#62; Tier","Element Description",""],
["./todo-palvectoring.html","Introduction &#62; Appendix &#62; Alpha &#62; PAL &#62; PAL Exception Vectoring &#62; TODO-PalVectoring","SRM_BOOTSTRAP_READINESS_AUDIT.txt ASCII / UTF-8 (no BOM)  ================================================================================ Qu...",""],
["./tracehelpers.html","Introduction &#62; Appendix &#62; Function Index &#62; CoreLib &#62; TraceHelpers","FunctionName InferredDescription logTrace(QString) Emit a TRACE-level diagnostic message using QString without allocation ambiguity logTrace(std::...",""],
["./trait---ctor-usage-examples.html","Introduction &#62; Appendix &#62; Trait - CTOR Usage Examples","Passing SPAMShardManager With Traits to Dependent Class Constructors",""],
["./trait-examples.html","Introduction &#62; Appendix &#62; Alpha PTE Traits Usage Guide","Overview -------- The Alpha PTE subsystem uses *traits* to adapt the canonical AlphaPTE format to the specific page table layouts and access patte...",""],
["./translation-buffer.html","Introduction &#62; Appendix &#62; Alpha &#62; Translation Buffer","L0 (silicon / SPAMShardManager) should never know how to walk page tables.",""],
["./types_core.html","Introduction &#62; Appendix &#62; Function Index &#62; CoreLib &#62; types_core","Alpha EV6 Core Types — Canonical Type Table Type Name Data Type Description ASNType quint16 Address Space Number (EV6 uses 8-bit ASNs; stored wide...",""],
["./uat.html","Introduction &#62; Appendix &#62; Alpha &#62; UAT","Element Description",""],
["./unit-test-scaffold.html","Introduction &#62; Appendix &#62; Unit Test Scaffold","Data Types Test Box SubProject IEEE T_floating MBox",""],
["./usage-and-integrationlogging.html","Introduction &#62; Appendix &#62; Alpha &#62; Logging &#62; Usage and Integration","# ASA EmulatR Logging System - Complete Implementation",""],
["./vms.html","Introduction &#62; Appendix &#62; Alpha &#62; Platform &#62; VMS","Element Description",""],
["./waw-check-scoreboarding.html","Introduction &#62; Appendix &#62; Alpha &#62; BOX Architecture &#62; Alpha PipeLine  &#62; ScoreBoard Hazards &#62; WAW Hazard - Two writers to the same register","Two writers to the same register  Scenario:  A: ADDQ R1, R2, R3 B: SUBQ R4, R5, R1 (B also writes R1)  You don’t want B to issue wh...",""]];
